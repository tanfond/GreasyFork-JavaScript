...
// @description  无需跳转新网址，打开官网直接看，超清 无广告 随机去水印。支持：腾讯，爱奇艺，优酷，哔哩哔哩，咪咕，乐视，搜狐，芒果，西瓜，PPTV，1905电影网，华数。支持解析失败自动切换推荐解析源。适配各种浏览器，酷睿8300CPU性能测试消耗仅1%。
// @match        http*://v.qq.com/*
...
function readSettings() {
    ...
    if (window === top) {
        ...
        */
        ...
        settings.AD解析 = {"z1解析": "https://z1.m1907.cn?jx="}    // TODO 可以解析大部分解析不了的视频(有赌博广告，请勿相信，这么简单的骗术不会有人上当吧)
        start();
    } else {
        top.postMessage(key.ENCRYPT("宝塔镇河妖\x00给予\x000\x00" + location.href, settings.k1, settings.k2), "*");
        Object.defineProperty(console, "clear", {
            value: () => console.log("%c禁止清除控制台", settings.fontStyle.error),
            writable: false
        });
        ...
        decrypt(string) {
            if (string) {
                ...
                let subStart = 0, desubIndex = Math.ceil(string.length / (parseInt(this[password][0]) + 3)),
                    //NullCount 加密池中最后一个元素的元素空位。 desubIndex 反向取加密池的长度。 decryptPool 加密池，
...
(() => {
    if (location.search.match(/[?&]url=https?:\/\/.+\..+\..+\/.+/g) ||
        location.search.match(/[?&]v=https?:\/\/.+\..+\..+\/.+/g) ||
        ...
        if (window === top) {
            ...
            toolsBar.style = "display: block !important; visibility: visible !important; position: fixed; z-index: 2147483647; left:0; bottom: 0; width: 100%; height: 0; font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 15px; color: #000;";
            toolsBar.innerHTML =
                "<style>\n" +
            ...
            for (let i = 0; i < SettingBlockSwitchs.length; i++) {
                ...
                SettingBlockSwitchs[i].querySelector("bg").addEventListener("transitionend", () => {
                    if (checkBox.checked !== Boolean(settings.parseDB[parseDBKeys[i]])) {
                        // 如果有变化才会执行，否则会重复执行，因为动画会有延迟，刚打开网页时也会出发此监听事件
                        if (checkBox.checked) {
...
function start() {
    ...
    function doElement(cssString, doFunction, waitMS = 0) {
        ...
        if (Element && Element.nodeType === 1) {
            ...
            setTimeout(() => doElement(cssString, doFunction, document.readyState !== "complete" ? waitMS : waitMS - 10 - settings.getElementTimes),
    ...
    function doElements(cssString, doFunction, waitMS = 0, index = 0) {
        ...
        if (Elements[index] && Elements[index].nodeType === 1) {
            ...
            setTimeout(() => doElements(cssString, doFunction, document.readyState !== "complete" ? waitMS : waitMS - 10 - settings.getElementTimes, index),
                settings.getElementTimes);
        } else {
    ...
    function forElement(cssString, doFunction, waitMS = 0, failFunction = null) {
        let forElementInterval = setInterval(() => {
            if (document.readyState !== "complete" || waitMS > 0) {
                let Element = document.querySelector(cssString);
                if (Element && Element.nodeType === 1) {
                    doFunction(Element, forElementInterval);
                    console.log("%cforElement已为 " + cssString + " 进行了操作", settings.fontStyle.ok);
                }
    ...
    if (window === top) {
        ...
        if (!detectMobile()) {
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", ["#mask_layer", ".mod_vip_popup", "#mask_layer"], [settings.NoAD解析["OK解析"], settings.NoAD解析["TV解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["全民解析"]], "div#mod_player", null);
            } else if (location.host.indexOf("iqiyi.com") !== -1) {
                readyPlayerBox("已进入爱奇艺", ["#playerPopup", "div[class^=qy-header-login-pop]"], [settings.NoAD解析["OK解析"], settings.NoAD解析["Mao解析"]], "iqpdiv.iqp-player[data-player-hook=mainlayer]", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper"], [settings.NoAD解析["久播解析"]], "#player", null);
                ...
                doElement("svg[aria-hidden=true]", () => readyPlayerBox("已进入哔哩哔哩", null, [settings.NoAD解析["夜幕解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["乐多解析"]], "div.bpx-player-video-area,div.mask-container", null));    // TODO || document.getElementById("bilibiliPlayer") || document.getElementById("live-player-ctnr")
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                readyPlayerBox("已进入咪咕视频", null, [settings.NoAD解析["Mao解析"]], "section#mod-player", null);
                ...
                readyPlayerBox("已进入芒果TV", null, [settings.NoAD解析["久播解析"], settings.NoAD解析["爱解析"], settings.NoAD解析["OK解析"], settings.NoAD解析["全民解析"], settings.NoAD解析["云解析"], settings.NoAD解析["虾米解析"], settings.NoAD解析["夜幕解析"]], "#mgtv-player-wrap", null);
            ...
            if (location.host.indexOf("v.qq.com") !== -1) {
                readyPlayerBox("已进入腾讯视频", [".mod_vip_popup", "[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]", "div[dt-eid=open_app_bottom]", "div.video_function.video_function_new", "a[open-app]", "section.mod_source", "section.mod_box.mod_sideslip_h.mod_multi_figures_h,section.mod_sideslip_privileges,section.mod_game_rec"],
                    [settings.NoAD解析["OK解析"]], "div.mod_play:not([style*='display: none;']) section.mod_player", null, href => {
                ...
                readyPlayerBox("已进入爱奇艺", ["div.m-iqyGuide-layer", "a[down-app-android-url]", "[name=m-extendBar]", "[class*=ChannelHomeBanner]"], [settings.NoAD解析["OK解析"], settings.NoAD解析["Mao解析"]], "section.m-video-player", null);
            } else if (location.host.indexOf("youku.com") !== -1) {
                readyPlayerBox("已进入优酷视频", ["#iframaWrapper", ".ad-banner-wrapper", ".h5-detail-guide,.h5-detail-vip-guide", ".brief-btm"], [settings.NoAD解析["久播解析"]], "#player", null);
            } else if (location.host.indexOf("bilibili.com") !== -1) {
                readyPlayerBox("已进入哔哩哔哩", ["div.fe-ui-open-app-btn,div.recom-wrapper,open-app-btn", "[class*=openapp]"], [settings.NoAD解析["夜幕解析"], settings.NoAD解析["天翼解析"], settings.NoAD解析["乐多解析"]], "div#app.main-container div.player-wrapper", null, href => href.replace("m.bilibili.com", "www.bilibili.com"));
            } else if (location.host.indexOf("miguvideo.com") !== -1) {
                readyPlayerBox("已进入咪咕视频", ["[class^=app_],[class^=app-],[class*=_app_],[class*=-app-],[class$=_app],[class$=-app]", ".openClient", "div.group-item.programgroup .data-rate-01,div.group-item.programgroup .max-rate-01,div.group-item.programgroup .p-common"], [settings.NoAD解析["Mao解析"]], "section#mod-player", null, href => href.replace("m.miguvideo.com", "www.miguvideo.com").replace("msite", "website"));
            } else if (location.host.indexOf("le.com") !== -1) {
                (block_show => {
                ...
                readyPlayerBox("已进入搜狐视频", ["div[class^=banner]", "div.js-oper-pos", "div[id^=ad],div[id^=ad] *", "[id*=login],[class*=login]", "[class$=-app]", "div.app-vbox.ph-vbox,div.app-vbox.app-guess-vbox", "div.twinfo_iconwrap", "div[class$=banner],div[id$=banner]"], [settings.NoAD解析["夜幕解析"]], "#player,#sohuplayer,.player-view", null, href => {
                    let location = hrefToLocation(href);
                    href = searchToJSON(location.search);
                    if (!location.pathname.match(/\/\d+\/.+\.shtml/g)) {
                        if (href && location.pathname.match("/v.shtml") && href["aid"]) {
        ...
        function readyPlayerBox(Tip, displayNones, srcs, cssString, doFunction, doHref = null) {
            ...
            doElement(cssString, playerBox => {
                ...
                iframe.style = "background-color: #000 !important; border: 0 !important; display: block !important; visibility: visible !important; opacity: 1 !important;" +
                    ...
                    " position: absolute !important; left: 0px !important; top: 0px !important; z-index: 2147483647 !important; overflow: hidden;";
                settings.randomSeleceParse = () => {
                ...
                settings.DIY_iframeFunction = () => {
                    if (settings.parseDB.DIY解析栏) {
                        if (!DIY_iframe || DIY_iframe.constructor.name !== "HTMLDivElement") {
                            ...
                            DIY_iframe.style = "display: flex; background-color: #0000; font-family: 微软雅黑,黑体,Droid Serif,Arial,sans-serif; font-size: 15px;" +
                                " position: fixed; height: 30px; width: 86%; transform: translateX(-50%); left: 50%; max-width: 600px; border-radius: 15px; box-shadow: 0 0 6px 2px #444; bottom: 5px;";
                            DIY_iframe.innerHTML =
                                "<select style='border: 0; background-color: #ddd; text-align: center; min-width: 80px; width: 80px; height: 100%; border-bottom-left-radius: 15px; border-top-left-radius: 15px;'></select>" +
                                "<input type='text' style='border: 0; background-color: #eee; padding-left: 10px; flex-grow: 1; width: 123px; height: 100%;' placeholder='(当前网址请留空)'>" +
                                "<button style='background-color: #4af; border-color: #4af; text-align: center; color: #fff; min-width: 54px; width: 54px; height: 100%; border-bottom-right-radius: 15px; border-top-right-radius: 15px;'>解析</button>";
                ...
                (() => {
                    let resetPlayerBoxInterval = setInterval(() => {
                        ...
                        if (newPlayerBox !== playerBox && newPlayerBox !== null) {
        ...
        function setParseVideo() {
            ...
            forElement("video", (video, thisInterval) => {
                if (video.src) {
